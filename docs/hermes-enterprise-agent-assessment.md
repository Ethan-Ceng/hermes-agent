...
| 适配层 | 主要职责 |
|---|---|
| 员工 Web 应用 | ChatGPT 式聊天、个人历史、流式回答、停止生成，以及按业务需要上传和下载文件 |
| 企业 Web 后端 | 登录、员工与 Hermes 分片/profile 的映射、会话和文件归属校验、任务记录 |
| 业务连接 | 带员工权限调用知识库或业务系统，处理结构化结果 |
| Skills / 插件 / MCP | 业务流程、工具连接、可复用文档处理能力 |
| 部署层 | 共享 Agent 服务、profile 持久化、按需 Sandbox、资源限制和日志 |
...
因此推荐边界是“一个员工一个 profile，一个对话一个 Hermes session”。对话消息、工具结果和压缩后的短期上下文按 session 分开；`MEMORY.md` 和 `USER.md` 按员工 profile 分开，并可由该员工的多个对话使用。内置记忆在会话开始时作为冻结快照进入系统提示，写入的新记忆不应在活动会话中强制重载，而应由之后的新会话读取，以保留稳定的提示缓存。
...
采用“登录员工 → 固定 Hermes 分片/profile → 个人状态”的映射。Web 后端根据认证结果选择内部 profile，员工不能通过修改请求中的 profile、session ID 或文件路径访问其他人的数据。

| 数据或能力 | 第一版边界 |
|---|---|
| 对话、历史和任务记录 | 每个对话对应员工 profile 内的独立 Hermes session；每次读取和控制任务都检查员工归属 |
| 内置记忆 | 使用员工自己的 Hermes home / profile；同一员工的多个对话可以共享，不能跨员工读取 |
| 工作文件 | 按员工命名空间保存，所有读取与下载都检查归属 |
| 代码、终端与浏览器执行 | 只在按任务分配的临时 Sandbox 中运行，不进入共享 Hermes 进程 |
...
默认采用 Hermes 已有的 multiplex：一个共享 Gateway / API Server 承载多个员工 profile，每个 profile 分开 SessionDB、记忆、配置和 secrets。企业 Web 后端维护员工到分片与 opaque profile 的固定映射，并使用 profile 自己的 API key 调用 `/p/<profile>/...`。[多 profile 运行文档](https://hermes-agent.nousresearch.com/docs/user-guide/multi-profile-gateways)

共享进程内必须关闭宿主机终端、跨 profile `session_search`、任意路径文件访问及未经验证的插件。代码执行和复杂文件处理优先复用 Hermes 已有 Docker backend，配置为非持久、无宿主机任意挂载、无长期凭据和受限网络；浏览器自动化需要时再接独立 Sandbox。这样新增员工主要增加 profile 存储和实际模型用量，不需要一个常驻容器，也不必在第一版另造 Sandbox 调度平台。

profile 仍不是对抗进程内恶意代码的安全边界。监管、高敏或需要更强隔离的用户组可以单独路由到专用 Hermes 分片或独立实例，但不应让所有普通员工承担这一固定成本。

## 6. 为什么第一版选择 Web

Web 适合当前试点：员工通过浏览器获得与 ChatGPT、千问、豆包相近的自然对话体验；集中发布也方便持续调整业务流程与评估指标。
...
第一版员工界面关注登录、个人历史、输入与流式回答、简单的“正在处理”状态和停止生成。允许范围内的工具在共享 Agent 服务或按需 Sandbox 中自动执行，不向员工展示命令、工具进度或技术审批；详细过程只进入后台日志与审计。文件上传与结果下载是否需要，取决于所选业务场景。
...
flowchart TD
    ...
    subgraph Server[企业服务器]
        ...
        Backend --> Agent[共享 Hermes Agent 服务]
        Agent --> ProfileA[员工 A Profile]
        Agent --> ProfileB[员工 B Profile]
        Agent --> Sandbox[按需 Sandbox]
    end
    Agent --> Model[DeepSeek 模型 API]
```

企业 Web 后端只提供一个公共入口，Hermes 后端接口留在受控网络内。每名员工拥有自己的逻辑状态空间，Hermes 计算和模型连接由多个员工共享；达到容量阈值后按员工组增加 Agent 分片，而不是按员工增加实例。

继续使用 DeepSeek API 时，无需在企业服务器部署模型。下载 PDF、安装依赖和运行 Python 等高风险执行发生在临时 Sandbox；普通对话和只读业务查询无需启动 Sandbox。Web 页面主要展示流式回答和最终结果，执行细节由后台记录。

### 7.2 可复用的服务接口

| 入口 | 形式与启动关系 | 适配方向 |
|---|---|---|
| API Server | HTTP + SSE；启用后随 `hermes gateway` 启动 | Web 后端提交任务、读取事件、中止及保持会话连续性 |
| `hermes serve` 后端 | JSON-RPC / WebSocket，无管理 SPA | 需要更细粒度会话控制和客户端交互时评估 |
...
建议先在一台 Linux 服务器上部署企业 Web 入口、一个共享 Hermes multiplex 服务和按需启用的小型 Sandbox 池，按真实并发和任务耗时决定后续扩容。

- 持久保存每个员工 profile 的 SessionDB 和记忆，验证重启后归属及状态正确。
- 共享 Hermes 进程只启用经过 profile-safety 验证的最小工具集合。
- Sandbox 镜像预装目标业务依赖，按任务限制文件、网络、CPU、内存和时长。
- 业务凭据不进入 Sandbox；员工级 MCP token 通过可信服务端上下文获得。
...
| 界面 | 登录、个人聊天、个人历史、流式回答、简单处理状态和停止生成 |
| 模型 | 一个模型服务，保留实际模型版本与费率记录 |
| 业务 | 一个从输入到结果交付的完整场景 |
| 隔离 | 每员工独立 profile 状态；高风险执行按任务进入 Sandbox；所有访问检查员工归属 |
...
4. **工具隔离**：共享 Agent 不暴露宿主机终端、跨 profile 搜索和任意文件访问；terminal / execute_code 强制使用受限 Docker backend，并验证无法读取另一员工、其他任务或管理环境的数据。
